...
do
	...
	local function parse_literal(s, lit, val)
		if s:sub(pos, pos + #lit - 1) == lit then
			pos = pos + #lit
			return val
		end
...
if #fixtures == 0 then
	fail("no fixtures found in " .. FIXTURE_DIR)
end

local fixture_types = {}
for _, name in ipairs(fixtures) do
	fixture_types[name:gsub("%.json$", "")] = true
end
...
for _, name in ipairs(fixtures) do
	...
	if expected_cb then
		...
		if not raw then
			...
			local rt = new_rt()
			local fired = false
			rt:on(expected_cb, function(_payload)
				fired = true
			end)
...
-- Typo at registration must blow up, not silently no-op. Guards the
-- breaking-API release where every user is retyping callback names.
do
	local rt = new_rt()
	local ok = pcall(function()
		rt:on("match_mached", function() end)
	end)
	if ok then
		fail("M:on accepted a typo'd event name 'match_mached'")
	else
		pass("M:on rejects unknown event name")
	end

	local ok2 = pcall(function()
		rt:on("entity_added", function() end)
	end)
	if ok2 then
		pass("M:on accepts known event 'entity_added'")
	else
		fail("M:on rejected known event 'entity_added'")
	end
end

print(
	string.format("[dispatch] %d passed, %d failed (%d fixtures)", pass_count, fail_count, #fixtures)
)
