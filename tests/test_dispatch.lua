...
-- game.error carries the dev-console payload (callback/script/message) that
-- the SDK is meant to surface, not just a bare fire-or-not signal. Assert
-- the fields actually reach the callback intact.
do
	...
	if not raw then
		...
		local rt = new_rt()
		local payload
		rt:on("game_error", function(p) payload = p end)
		rt:_handle_message(raw)
		if not payload then
