...
-- real encoder in tests/fixtures/wire, with a manifest saying what each one
...
--
-- This replaces the test for the old asobi_ws_binary sub-protocol, which the
-- server no longer speaks.
...
local FIXTURE_DIR = "tests/fixtures/wire"
...
-- A JSON frame must never be mistaken for a binary one. The old sub-protocol
-- sniffed for a leading byte in {0x01, 0x02, 0x03}, and the new frame-kind byte
-- is 1 or 2 - so the test is "is it '{'", nothing else.
do
...
do
	...
	rt:_handle_message(read_file(FIXTURE_DIR .. "/keyframe_all_adds.bin"))
	local count = 0
