...
-- the bytes.
--
-- Arithmetic only: no `string.unpack`, no `bit32`, no bitwise operators. Defold
-- runs Lua 5.1 on HTML5 and LuaJIT elsewhere, so none of those exist on every
-- target this SDK has to work on. Same constraint as asobi/device.lua.
--
...
-- A frame is binary when it is not JSON. Sniffing for a specific set of leading
-- bytes is what the previous binary sub-protocol did, and it broke the moment the
-- frame kinds happened to overlap that set. A JSON frame from this server always
-- starts with '{', so test for that and nothing else.
